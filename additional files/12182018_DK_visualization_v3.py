...
# loan2["loan_status_closed"] = ((loan2["loan_status"]=="A")|(loan2["loan_status"]=="B")).astype(int)
...
#revised one
loan_amount_all= loan2.drop(["monthly_loan_payment", "num_days_since_grant", "loan_status_closed","loan_duration"], axis=1)
loan_duration_all=loan2.drop(["loan_amount", "loan_status_closed","num_days_since_grant","monthly_loan_payment"], axis=1)
loan_since_grant=loan2.drop(["monthly_loan_payment","loan_amount","loan_status_closed","loan_duration"], axis=1)
...
plt.savefig('fig_'+str('age_n_loanamount')+'.png', dpi=200)
plt.show()
...
ax = basetabledeb.groupby(by=['loan_status']).mean()["monthly_loan_payment"].plot.bar(legend=False)
plt.xlabel('Loan Status', fontweight='bold', color = 'blue', fontsize='10', horizontalalignment='center')
plt.ylabel("Monthly Loan Payment")
plt.title("Age and Monthly Loan Payment by Loan Status")
plt.xticks(rotation=30)
#plt.plot(basetabledeb['client_age'])
